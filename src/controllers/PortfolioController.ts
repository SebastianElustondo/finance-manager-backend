...
export class PortfolioController {
  ...
  getPortfolioById = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      try {
        ...
        if (serviceError.message === 'Portfolio not found') {
  ...
  createPortfolio = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      try {
        ...
        if (serviceError.message.includes('required') || serviceError.message.includes('name')) {
          return res.status(400).json({
            success: false,
  ...
  updatePortfolio = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      try {
        const portfolio = await this.portfolioService.updatePortfolio(portfolioId, userId, {
          name,
          description,
          currency,
          isDefault,
